...
**Window → Xestrel → Isolation Graph**(または分離ウィンドウの **Graph** ボタン)で、分離状態の読み取り専用 GraphView が開きます: アバター → マテリアル → テクスチャ、アバター → Descriptor のコントローラ → クリップ。ノードは 橙(まだ共有)/ 緑(隔離済みコピー)/ 赤(他ワークスペースのマニフェストも参照している隔離コピー — Fork が必要な複製アバター。シーンが未ロードでも検出されます)で色分けされます。ノードをダブルクリックするとアセットを Ping します。
