...
**Window → Xestrel → Dependency Graph**(または分離ウィンドウの **Graph** ボタン)で、アバターの依存関係グラフが開きます。第1リングは汎用的に発見されます — 階層内の全コンポーネントのシリアライズ済みオブジェクト参照をすべて走査するため、Modular Avatar / VRCFury / オーディオ / メッシュ / メニューへの参照も型ごとの対応なしで現れます。各ノードの **+n** ボタンでそのノード自身の直接依存が展開され(インポートパイプライン経由なので軽量)、初期表示は小さく保たれます。種別トグル(Mat / Tex / Mesh / Anim / Clip / Menu / Shader / Prefab / Other)、**Hide Packages** トグル、検索ハイライトも利用できます。分離状態は色のオーバーレイです: 橙=まだ共有(隔離可能)/ 緑=隔離済みコピー / 赤=他ワークスペースのマニフェストも参照しているコピー(Fork が必要な複製アバター。未ロードのシーンでも検出)/ グレー=xestrel の管理対象外。ノードを右クリックでプライマリアバターに対する **Isolate** / **Restore**、ダブルクリックで Ping。**Add Selected** で複数のアバターをルートに追加すると、アバター間のアセット共有が面で見えます。
